...
#include <fstream> // for writing the json file
...
class generate_iec_c: public visitor_c {
  private:
    stage4out_c &s4o;
    json *s4json;
    json::json_pointer *s4json_pointer;

  public:
    generate_iec_c(json *s4json_, json::json_pointer *s4json_pointer_,  stage4out_c *s4o_ptr): s4o(*s4o_ptr) {
      s4json = s4json_;
      s4json_pointer = s4json_pointer_;
    }
    ~generate_iec_c(void) {}
...
void after_to_json_call(){
  // go up two levels in the json tree.
  // In practice this from body/0/body/1 to body/0
  s4json_pointer->pop_back();
  s4json_pointer->pop_back();
}

// add token to json
void token_to_json(std::string name, token_c *token){
  // go to the body of the current json object
  *s4json_pointer /= "nodes";

  // """ 
  // We first dereference the s4json_pointer pointer to obtain the json_pointer object, 
  // and use it as the index for dereferencing the s4json pointer to obtain the json object. 
  // We then use the push_back method to append a new object to the JSON list
  // """
  (*s4json)[*s4json_pointer].push_back(
    {
      {"name", name},
      {"category", "token"},
...
// add list to json
// todo: list to json is not called anywhere
void list_to_json(std::string name, list_c *list,
...
void declaration_to_json(std::string name, symbol_c *symbol){
  // add a decleration to the body of the current json object

  // go to the body of the current json object
  *s4json_pointer /= "nodes";

  // s4o.print("\n\n");
  // s4o.print((*s4json_pointer).to_string()); s4o.print("\n");

  std::string new_index = "0";
  if (!(*s4json)[*s4json_pointer].empty()){
    new_index = std::to_string((*s4json)[*s4json_pointer].size()); // get the index of the new object
  }
  ...
  // go to the added object
  *s4json_pointer /= new_index;
  
  
  // // retrieve the serialized value (explicit JSON serialization)
  // std::string serialized_string = s4json->dump(2, ' ', true, json::error_handler_t::replace);
  // s4o.print(serialized_string);
  // s4o.print("\n\n");
}
...
void *visit(pragma_c *symbol)                  {token_to_json("pragma", symbol); return print_token(symbol);}
...
/*******************************************/
void *visit(                 identifier_c *symbol) {token_to_json("identifier", symbol); return print_token(symbol);}
void *visit(derived_datatype_identifier_c *symbol) {token_to_json("derived_datatype_identifier", symbol); return print_token(symbol);}
void *visit(         poutype_identifier_c *symbol) {token_to_json("poutype_identifier", symbol); return print_token(symbol);}
...
/******************************/
void *visit(real_c *symbol)               {token_to_json("real", symbol); return print_token(symbol);}
void *visit(neg_real_c *symbol) {
  declaration_to_json("neg_real", symbol);
...
void *visit(integer_c *symbol)            {token_to_json("integer", symbol); return print_token(symbol);}
void *visit(neg_integer_c *symbol) {
  declaration_to_json("neg_integer", symbol);
...
void *visit(binary_integer_c *symbol)     {token_to_json("binary_integer", symbol); return print_token(symbol);}
void *visit(octal_integer_c *symbol)      {token_to_json("octal_integer", symbol); return print_token(symbol);}
void *visit(hex_integer_c *symbol)        {token_to_json("hex_integer", symbol); return print_token(symbol);}

void *visit(integer_literal_c *symbol) {
  declaration_to_json("integer_literal", symbol);

  print_const_value(symbol->value);
  if (NULL != symbol->type) {
...
void *visit(real_literal_c *symbol) {
  declaration_to_json("real_literal", symbol);
...
void *visit(bit_string_literal_c *symbol) {
  declaration_to_json("bit_string_literal", symbol);
...
void *visit(boolean_literal_c *symbol) {
  declaration_to_json("boolean_literal", symbol);
...
void *visit(boolean_true_c *symbol)       {s4o.print(/*"TRUE" */"1"); token_to_json("boolean_true", nullptr); return NULL;}
void *visit(boolean_false_c *symbol)      {s4o.print(/*"FALSE"*/"0"); token_to_json("boolean_false", nullptr); return NULL;}

/*******************************/
/* B.1.2.2   Character Strings */
/*******************************/
void *visit(double_byte_character_string_c *symbol) {token_to_json("double_byte_character_string", symbol); return print_token(symbol);}
void *visit(single_byte_character_string_c *symbol) {token_to_json("single_byte_character_string", symbol); return print_token(symbol);}
...
void *visit(neg_time_c *symbol) {
  token_to_json("neg_time", nullptr);
  s4o.print("-"); 
  return NULL;
}

void *visit(duration_c *symbol) {
  declaration_to_json("duration", symbol);
...
void *visit(fixed_point_c *symbol) {token_to_json("fixed_point", symbol); return print_token(symbol);}

/* SYM_REF5(interval_c, days, hours, minutes, seconds, milliseconds) */
void *visit(interval_c *symbol) {
  declaration_to_json("interval", symbol);
...
void *visit(time_of_day_c *symbol) {
  declaration_to_json("time_of_day", symbol);
...
void *visit(daytime_c *symbol) {
  declaration_to_json("daytime", symbol);
...
void *visit(date_c *symbol) {
  declaration_to_json("date", symbol);
...
void *visit(date_literal_c *symbol) {
  declaration_to_json("date_literal", symbol);
...
void *visit(date_and_time_c *symbol) {
  declaration_to_json("date_and_time", symbol);
...
/***********************************/
void *visit(       time_type_name_c *symbol) {s4o.print("TIME");        token_to_json("time_type_name", nullptr); return NULL;}
void *visit(       bool_type_name_c *symbol) {s4o.print("BOOL");        token_to_json("bool_type_name", nullptr); return NULL;}
void *visit(       sint_type_name_c *symbol) {s4o.print("SINT");        token_to_json("sint_type_name", nullptr); return NULL;}
void *visit(        int_type_name_c *symbol) {s4o.print("INT");         token_to_json("int_type_name", nullptr); return NULL;}
void *visit(       dint_type_name_c *symbol) {s4o.print("DINT");        token_to_json("dint_type_name", nullptr); return NULL;}
void *visit(       lint_type_name_c *symbol) {s4o.print("LINT");        token_to_json("lint_type_name", nullptr); return NULL;}
void *visit(      usint_type_name_c *symbol) {s4o.print("USINT");       token_to_json("usint_type_name", nullptr); return NULL;}
void *visit(       uint_type_name_c *symbol) {s4o.print("UINT");        token_to_json("uint_type_name", nullptr); return NULL;}
void *visit(      udint_type_name_c *symbol) {s4o.print("UDINT");       token_to_json("udint_type_name", nullptr); return NULL;}
void *visit(      ulint_type_name_c *symbol) {s4o.print("ULINT");       token_to_json("ulint_type_name", nullptr); return NULL;}
void *visit(       real_type_name_c *symbol) {s4o.print("REAL");        token_to_json("real_type_name", nullptr); return NULL;}
void *visit(      lreal_type_name_c *symbol) {s4o.print("LREAL");       token_to_json("lreal_type_name", nullptr); return NULL;}
void *visit(       date_type_name_c *symbol) {s4o.print("DATE");        token_to_json("date_type_name", nullptr); return NULL;}
void *visit(        tod_type_name_c *symbol) {s4o.print("TOD");         token_to_json("tod_type_name", nullptr); return NULL;}
void *visit(         dt_type_name_c *symbol) {s4o.print("DT");          token_to_json("dt_type_name", nullptr); return NULL;}
void *visit(       byte_type_name_c *symbol) {s4o.print("BYTE");        token_to_json("byte_type_name", nullptr); return NULL;}
void *visit(       word_type_name_c *symbol) {s4o.print("WORD");        token_to_json("word_type_name", nullptr); return NULL;}
void *visit(      lword_type_name_c *symbol) {s4o.print("LWORD");       token_to_json("lword_type_name", nullptr); return NULL;}
void *visit(      dword_type_name_c *symbol) {s4o.print("DWORD");       token_to_json("dword_type_name", nullptr); return NULL;}
void *visit(     string_type_name_c *symbol) {s4o.print("STRING");      token_to_json("string_type_name", nullptr); return NULL;}
void *visit(    wstring_type_name_c *symbol) {s4o.print("WSTRING");     token_to_json("wstring_type_name", nullptr); return NULL;}

void *visit(       void_type_name_c *symbol) {s4o.print("VOID");        token_to_json("void_type_name", nullptr); return NULL;} /* a non-standard extension! */

void *visit(   safetime_type_name_c *symbol) {s4o.print("SAFETIME");    token_to_json("safetime_type_name", nullptr); return NULL;}
void *visit(   safebool_type_name_c *symbol) {s4o.print("SAFEBOOL");    token_to_json("safebool_type_name", nullptr); return NULL;}
void *visit(   safesint_type_name_c *symbol) {s4o.print("SAFESINT");    token_to_json("safesint_type_name", nullptr); return NULL;}
void *visit(    safeint_type_name_c *symbol) {s4o.print("SAFEINT");     token_to_json("safeint_type_name", nullptr); return NULL;}
void *visit(   safedint_type_name_c *symbol) {s4o.print("SAFEDINT");    token_to_json("safedint_type_name", nullptr); return NULL;}
void *visit(   safelint_type_name_c *symbol) {s4o.print("SAFELINT");    token_to_json("safelint_type_name", nullptr); return NULL;}
void *visit(  safeusint_type_name_c *symbol) {s4o.print("SAFEUSINT");   token_to_json("safeusint_type_name", nullptr); return NULL;}
void *visit(   safeuint_type_name_c *symbol) {s4o.print("SAFEUINT");    token_to_json("safeuint_type_name", nullptr); return NULL;}
void *visit(  safeudint_type_name_c *symbol) {s4o.print("SAFEUDINT");   token_to_json("safeudint_type_name", nullptr); return NULL;}
void *visit(  safeulint_type_name_c *symbol) {s4o.print("SAFEULINT");   token_to_json("safeulint_type_name", nullptr); return NULL;}
void *visit(   safereal_type_name_c *symbol) {s4o.print("SAFEREAL");    token_to_json("safereal_type_name", nullptr); return NULL;}
void *visit(  safelreal_type_name_c *symbol) {s4o.print("SAFELREAL");   token_to_json("safelreal_type_name", nullptr); return NULL;}
void *visit(   safedate_type_name_c *symbol) {s4o.print("SAFEDATE");    token_to_json("safedate_type_name", nullptr); return NULL;}
void *visit(    safetod_type_name_c *symbol) {s4o.print("SAFETOD");     token_to_json("safetod_type_name", nullptr); return NULL;}
void *visit(     safedt_type_name_c *symbol) {s4o.print("SAFEDT");      token_to_json("safedt_type_name", nullptr); return NULL;}
void *visit(   safebyte_type_name_c *symbol) {s4o.print("SAFEBYTE");    token_to_json("safebyte_type_name", nullptr); return NULL;}
void *visit(   safeword_type_name_c *symbol) {s4o.print("SAFEWORD");    token_to_json("safeword_type_name", nullptr); return NULL;}
void *visit(  safelword_type_name_c *symbol) {s4o.print("SAFELWORD");   token_to_json("safelword_type_name", nullptr); return NULL;}
void *visit(  safedword_type_name_c *symbol) {s4o.print("SAFEDWORD");   token_to_json("safedword_type_name", nullptr); return NULL;}
void *visit( safestring_type_name_c *symbol) {s4o.print("SAFESTRING");  token_to_json("safestring_type_name", nullptr); return NULL;}
void *visit(safewstring_type_name_c *symbol) {s4o.print("SAFEWSTRING"); token_to_json("safewstring_type_name", nullptr); return NULL;}

/********************************/
/* B.1.3.2 - Generic data types */
/********************************/
void *visit(generic_type_any_c      *symbol) {s4o.print("ANY");         token_to_json("generic_type_any", nullptr); return NULL;}
...
void *visit(data_type_declaration_c *symbol) {
  declaration_to_json("data_type_declaration", symbol);
...
void *visit(simple_type_declaration_c *symbol) {  
  declaration_to_json("simple_type_declaration", symbol);
...
void *visit(simple_spec_init_c *symbol) {
  declaration_to_json("simple_spec_init", symbol);
...
void *visit(subrange_type_declaration_c *symbol) {
  declaration_to_json("subrange_type_declaration", symbol);
...
void *visit(subrange_spec_init_c *symbol) {
  declaration_to_json("subrange_spec_init", symbol);
...
void *visit(enumerated_type_declaration_c *symbol) {
  declaration_to_json("enumerated_type_declaration", symbol);
...
void *visit(enumerated_spec_init_c *symbol) {
  declaration_to_json("enumerated_spec_init", symbol);
...
void *visit(enumerated_value_c *symbol) {
  declaration_to_json("enumerated_value", symbol);
...
void *visit(array_type_declaration_c *symbol) {
  declaration_to_json("array_type_declaration", symbol);
...
void *visit(array_spec_init_c *symbol) {
  declaration_to_json("array_spec_init", symbol);
...
void *visit(array_specification_c *symbol) {
  declaration_to_json("array_specification", symbol);
...
void *visit(array_initial_elements_c *symbol) {
  declaration_to_json("array_initial_elements", symbol);
...
void *visit(structure_type_declaration_c *symbol) {
  declaration_to_json("structure_type_declaration", symbol);
...
void *visit(initialized_structure_c *symbol) {
  declaration_to_json("initialized_structure", symbol);
...
void *visit(structure_element_declaration_list_c *symbol) {
  declaration_to_json("structure_element_declaration_list", symbol);
...
void *visit(structure_element_declaration_c *symbol) {
  declaration_to_json("structure_element_declaration", symbol);
...
void *visit(structure_element_initialization_c *symbol) {
  declaration_to_json("structure_element_initialization", symbol);
...
void *visit(string_type_declaration_c *symbol) {
  declaration_to_json("string_type_declaration", symbol);
...
void *visit(fb_spec_init_c *symbol) {
  declaration_to_json("fb_spec_init", symbol);
...
void *visit(ref_spec_c *symbol) {
  declaration_to_json("ref_spec", symbol);
...
void *visit(ref_spec_init_c *symbol) {
  declaration_to_json("ref_spec_init", symbol);
...
void *visit(ref_type_decl_c *symbol) {
  declaration_to_json("ref_type_decl", symbol);
...
void *visit(symbolic_variable_c *symbol) {
  declaration_to_json("symbolic_variable", symbol);
...
void *visit(symbolic_constant_c *symbol) {
  declaration_to_json("symbolic_constant", symbol);
...
void *visit(direct_variable_c *symbol) {
  token_to_json("direct_variable", symbol); 
...
void *visit(array_variable_c *symbol) {
  declaration_to_json("array_variable_c", symbol);
...
void *visit(structured_variable_c *symbol) {
  declaration_to_json("structured_variable", symbol);
...
void *visit(input_declarations_c *symbol) {
  if (typeid(*(symbol->method)) == typeid(explicit_definition_c)) {
    declaration_to_json("input_declarations", symbol);
...
void *visit(edge_declaration_c *symbol) {
  declaration_to_json("edge_declaration", symbol);
...
void *visit(en_param_declaration_c *symbol) {
  if (typeid(*(symbol->method)) == typeid(explicit_definition_c)) {
    declaration_to_json("en_param_declaration", symbol);
...
void *visit(eno_param_declaration_c *symbol) {
  if (typeid(*(symbol->method)) == typeid(explicit_definition_c)) {
    declaration_to_json("eno_param_declaration", symbol);
...
void *visit(var1_init_decl_c *symbol) {
  declaration_to_json("var1_init_decl", symbol);

  s4o.print(" SIMPLE TYPE DECLERATION ");
  symbol->var1_list->accept(*this);
  s4o.print(" : ");
  symbol->spec_init->accept(*this);
...
void *visit(extensible_input_parameter_c *symbol) {
  declaration_to_json("extensible_input_parameter", symbol);
...
void *visit(array_var_init_decl_c *symbol) {
  declaration_to_json("array_var_init_decl", symbol);
...
void *visit(structured_var_init_decl_c *symbol) {
  declaration_to_json("structured_var_init_decl", symbol);
...
void *visit(fb_name_decl_c *symbol) {
  declaration_to_json("fb_name_decl", symbol);
...
void *visit(output_declarations_c *symbol) {
  if (typeid(*(symbol->method)) == typeid(explicit_definition_c)) {
    declaration_to_json("output_declarations", symbol);
...
void *visit(input_output_declarations_c *symbol) {
  declaration_to_json("input_output_declarations", symbol);
...
void *visit(array_var_declaration_c *symbol) {
  declaration_to_json("array_var_declaration", symbol);
...
void *visit(structured_var_declaration_c *symbol) {
  declaration_to_json("structured_var_declaration", symbol);
...
void *visit(var_declarations_c *symbol) {
  declaration_to_json("var_declarations", symbol);
...
void *visit(retentive_var_declarations_c *symbol) {
  declaration_to_json("retentive_var_declarations", symbol);
...
void *visit(located_var_declarations_c *symbol) {
  declaration_to_json("located_var_declarations", symbol);
...
void *visit(located_var_decl_c *symbol) {
  declaration_to_json("located_var_decl", symbol);
...
void *visit(external_var_declarations_c *symbol) {
  declaration_to_json("external_var_declarations", symbol);
...
void *visit(external_declaration_c *symbol) {
  declaration_to_json("external_declaration", symbol);
...
void *visit(global_var_declarations_c *symbol) {
  declaration_to_json("global_var_declarations", symbol);
...
void *visit(global_var_decl_c *symbol) {
  declaration_to_json("global_var_decl", symbol);
...
void *visit(global_var_spec_c *symbol) {
  declaration_to_json("global_var_spec", symbol);

  declaration_to_json("function_declaration", symbol);
  symbol->global_var_name->accept(*this);
  s4o.print(" ");
  symbol->location->accept(*this);
...
void *visit(location_c *symbol) {
  declaration_to_json("location", symbol);
...
void *visit(single_byte_string_var_declaration_c *symbol) {
  declaration_to_json("single_byte_string_var_declaration", symbol);
...
void *visit(single_byte_string_spec_c *symbol) {
  declaration_to_json("single_byte_string_spec", symbol);
...
void *visit(single_byte_limited_len_string_spec_c *symbol) {
  declaration_to_json("single_byte_limited_len_string_spec", symbol);
...
void *visit(double_byte_string_var_declaration_c *symbol) {
  declaration_to_json("double_byte_string_var_declaration", symbol);
...
void *visit(double_byte_string_spec_c *symbol) {
  declaration_to_json("double_byte_string_spec", symbol);
...
void *visit(double_byte_limited_len_string_spec_c *symbol) {
  declaration_to_json("double_byte_limited_len_string_spec", symbol);
...
void *visit(incompl_located_var_declarations_c *symbol) {
  declaration_to_json("incompl_located_var_declarations", symbol);
...
void *visit(incompl_located_var_decl_c *symbol) {
  declaration_to_json("incompl_located_var_decl", symbol);
...
void *visit(incompl_location_c *symbol) {
  token_to_json("incompl_location", symbol); 
  s4o.print(" AT ");
  return print_token(symbol);
}
...
void *visit(function_var_decls_c *symbol) {
  declaration_to_json("function_var_decls", symbol);
...
void *visit(function_block_declaration_c *symbol) {
  declaration_to_json("function_block_declaration", symbol);
...
void *visit(temp_var_decls_c *symbol) {
  declaration_to_json("temp_var_decls", symbol);
...
void *visit(non_retentive_var_decls_c *symbol) {
  declaration_to_json("non_retentive_var_decls", symbol);
...
void *visit(program_declaration_c *symbol) {
  declaration_to_json("program_declaration", symbol);
...
void *visit(initial_step_c *symbol) {
  declaration_to_json("initial_step", symbol);
...
void *visit(step_c *symbol) {
  declaration_to_json("step", symbol);
...
void *visit(action_association_c *symbol) {
  declaration_to_json("action_association", symbol);
...
void *visit(action_qualifier_c *symbol) {
  declaration_to_json("action_qualifier", symbol);
...
void *visit(qualifier_c *symbol) {
  token_to_json("qualifier", symbol); 
  print_token(symbol);
  return NULL;
}
...
void *visit(timed_qualifier_c *symbol) {
  token_to_json("timed_qualifier", symbol); 
  print_token(symbol);
  return NULL;
}
...
void *visit(transition_c *symbol) {
  declaration_to_json("transition", symbol);
...
void *visit(transition_condition_c *symbol) {
  declaration_to_json("transition_condition", symbol);
...
void *visit(steps_c *symbol) {
  declaration_to_json("steps", symbol);
...
void *visit(action_c *symbol) {
  declaration_to_json("action", symbol);
...
void *visit(configuration_declaration_c *symbol) {
  declaration_to_json("configuration_declaration", symbol);
...
void *visit(resource_declaration_c *symbol) {
  declaration_to_json("resource_declaration", symbol);
...
void *visit(single_resource_declaration_c *symbol) {
  declaration_to_json("single_resource_declaration", symbol);
...
void *visit(global_var_reference_c *symbol) {
  declaration_to_json("global_var_reference", symbol);
...
void *visit(program_output_reference_c *symbol) {
  declaration_to_json("program_output_reference", symbol);
...
void *visit(task_configuration_c *symbol) {
  declaration_to_json("task_configuration", symbol);
...
void *visit(task_initialization_c *symbol) {
  declaration_to_json("task_initialization", symbol);
...
void *visit(program_configuration_c *symbol) {
  declaration_to_json("program_configuration", symbol);
...
void *visit(fb_task_c *symbol) {
  declaration_to_json("fb_task", symbol);
...
void *visit(prog_cnxn_assign_c *symbol) {
  declaration_to_json("prog_cnxn_assign", symbol);
...
void *visit(prog_cnxn_sendto_c *symbol) {
  declaration_to_json("prog_cnxn_sendto", symbol);
...
void *visit(instance_specific_initializations_c *symbol) {
  declaration_to_json("instance_specific_initializations", symbol);
...
void *visit(instance_specific_init_c *symbol) {
  declaration_to_json("instance_specific_init", symbol);
...
void *visit(fb_initialization_c *symbol) {
  declaration_to_json("fb_initialization", symbol);
...
void *visit(il_instruction_c *symbol) {
  declaration_to_json("il_instruction_c", symbol);
...
void *visit(il_simple_operation_c *symbol) {
  declaration_to_json("il_simple_operation", symbol);
...
void *visit(il_function_call_c *symbol) {
  declaration_to_json("il_function_call", symbol);
...
void *visit(il_expression_c *symbol) {
  ...
  declaration_to_json("il_expression", symbol);
...
void *visit(il_jump_operation_c *symbol) {
  declaration_to_json("il_jump_operation_c", symbol);
...
void *visit(il_fb_call_c *symbol) {
  declaration_to_json("il_fb_call", symbol);
...
void *visit(il_formal_funct_call_c *symbol) {
  declaration_to_json("il_formal_funct_call", symbol);
...
void *visit(il_simple_instruction_c *symbol)	{
  declaration_to_json("il_simple_instruction", symbol);
...
void *visit(il_param_assignment_c *symbol) {
  declaration_to_json("il_param_assignment", symbol);
...
void *visit(il_param_out_assignment_c *symbol) {
  declaration_to_json("il_param_out_assignment", symbol);
...
void *visit(il_assign_operator_c *symbol) {
  declaration_to_json("il_assign_operator", symbol);
...
void *visit(il_assign_out_operator_c *symbol) {
  declaration_to_json("il_assign_out_operator_c", symbol);
...
void *visit( deref_operator_c   *symbol) {
  declaration_to_json("deref_operator", symbol);

  symbol->exp->accept(*this); s4o.print("^");

  after_to_json_call();
  return NULL;
}                      /* an extension to the IEC 61131-3 standard - based on the IEC 61131-3 v3 standard. ^      -> dereferences an address into a variable! */
void *visit( deref_expression_c *symbol) {
  declaration_to_json("deref_expression", symbol);
  ...
  after_to_json_call();
  return NULL;
} /* an extension to the IEC 61131-3 standard - based on the IEC 61131-3 v3 standard. DREF() -> dereferences an address into a variable! */
void *visit(   ref_expression_c *symbol) {
  declaration_to_json("ref_expression_c", symbol);
  ...
  after_to_json_call();
  return NULL;
} /* an extension to the IEC 61131-3 standard - based on the IEC 61131-3 v3 standard. REF()  -> returns address of the varible! */
void *visit(    or_expression_c *symbol) {
  declaration_to_json("or_expression", symbol);
...
void *visit(   xor_expression_c *symbol) {
  declaration_to_json("xor_expression", symbol);
...
void *visit(   and_expression_c *symbol) {
  declaration_to_json("and_expression", symbol);
...
void *visit(   equ_expression_c *symbol) {
  declaration_to_json("equ_expression", symbol);
...
void *visit(notequ_expression_c *symbol) {
  declaration_to_json("notequ_expression", symbol);
...
void *visit(    lt_expression_c *symbol) {
  declaration_to_json("lt_expression", symbol);
...
void *visit(    gt_expression_c *symbol) {
  declaration_to_json("gt_expression", symbol);
...
void *visit(    le_expression_c *symbol) {
  declaration_to_json("le_expression", symbol);
...
void *visit(    ge_expression_c *symbol) {
  declaration_to_json("ge_expression", symbol);
...
void *visit(   add_expression_c *symbol) {
  declaration_to_json("add_expression", symbol);
...
void *visit(   sub_expression_c *symbol) {
  declaration_to_json("sub_expression", symbol);
...
void *visit(   mul_expression_c *symbol) {
  declaration_to_json("mul_expression", symbol);
...
void *visit(   div_expression_c *symbol) {
  declaration_to_json("div_expression", symbol);
...
void *visit(   mod_expression_c *symbol) {
  declaration_to_json("mod_expression", symbol);
...
void *visit( power_expression_c *symbol) {
  declaration_to_json("power_expression", symbol);
...
void *visit(   neg_expression_c *symbol) {
  declaration_to_json("neg_expression", symbol);
...
void *visit(   not_expression_c *symbol) {
  declaration_to_json("not_expression", symbol);
...
void *visit(function_invocation_c *symbol) {
  declaration_to_json("function_invocation", symbol);
...
void *visit( assignment_statement_c *symbol) {
  declaration_to_json("assignment_statement", symbol);
...
void *visit(fb_invocation_c *symbol) {
  declaration_to_json("fb_invocation", symbol);
...
void *visit(input_variable_param_assignment_c *symbol) {
  declaration_to_json("input_variable_param_assignment", symbol);
...
void *visit(output_variable_param_assignment_c *symbol) {
  declaration_to_json("output_variable_param_assignment", symbol);
...
void *visit(if_statement_c *symbol) {
  declaration_to_json("if_statement", symbol);
...
void *visit(elseif_statement_c *symbol) {
  declaration_to_json("elseif_statement_c", symbol);
...
void *visit(case_statement_c *symbol) {
  declaration_to_json("case_statement", symbol);
...
void *visit(case_element_c *symbol) {
  declaration_to_json("case_element", symbol);
...
void *visit(for_statement_c *symbol) {
  declaration_to_json("for_statement", symbol);
...
void *visit(while_statement_c *symbol) {
  declaration_to_json("while_statement", symbol);
...
void *visit(repeat_statement_c *symbol) {
  declaration_to_json("repeat_statement", symbol);
...
class generate_abs_c: public null_visitor_c {
  ...
  public:
    ...
    // create a class object and pass json a pointer
    generate_iec_c  generate_iec              (&s4json, &s4json_pointer, &s4o);
    symbol->accept(generate_iec);
    
    // write the JSON object to a file
    std::ofstream output_file("output.json");
    output_file << std::setw(2) << s4json << std::endl;
    output_file.close();
...
visitor_c *new_code_generator(stage4out_c *s4o, const char *builddir)  {return new generate_abs_c(s4o);}
